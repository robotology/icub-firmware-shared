...
/** @fn         extern eOresult_t eom_ipnet_ResolveIP(EOMtheIPnet *ip, eOipv4addr_t ipaddr, eOreltime_t tout)
    @brief      it asks that IPnet.run to resolve IP (via ARP) in tout usec and the caller task is stopped until
...
// it asks that IPnet.run to resolve IP in tout usec but the caller task is not stopped. 
