...
{   // very important: fill table with order of eOerror_value_CFG_t
    ...
    {eoerror_value_CFG_mc_advfoc_failed_ICCping, "CFG: EOtheMotionController: ICCping fails, the application on the other core does not respond to the ping. ",""},
    {eoerror_value_CFG_mc_advfoc_ICCdiscovery_invalid, "CFG: ICCdiscovery detected on ","In par16: target board type in 0xff00 and number of invalid in 0x000f. In par64: each nibble contains 0x0 if ok, mask 0x1 if wrong type, mask 0x2 if wrong fw, mask 0x4 if wrong prot"}
