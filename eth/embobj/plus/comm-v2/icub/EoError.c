...
{   // very important: fill table with order of eOerror_value_CFG_t
    ...
    {eoerror_value_CFG_encoders_ok, "CFG: EOtheEncoderReader can be correctly configured. in p16 the number of joints (lsb) and failure mask (msb), in p64 for each physical joint a byte with (port, errorcode)"},
    {eoerror_value_CFG_encoders_failed_verify, "CFG: EOtheEncodeerReader cannot be configured. verification fails. in p16 the number of joints (lsb) and failure mask (msb), in p64 for each physical joint a byte with (port, errorcode)"},        
